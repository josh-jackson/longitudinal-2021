...
```{r}
long<- wide %>% 
  pivot_longer(cols = ext_1:ext_3, names_to = "time", values_to = "EXT") %>% separate(time, into = c("variable", "time"))
long
```
...
```{r}
wide %>% 
  pivot_longer(cols = ext_1:ext_3, names_to = "time", values_to = "EXT", names_prefix = "ext_") 
```
...
```{r}
wide %>% 
  pivot_longer(cols = ext_1:ext_3, names_to = "time", values_to = "EXT", names_prefix = "ext_", names_transform = list(time = as.numeric)) 
```
...
```{r}
soep$C.2005 <- with(soep, rowMeans(cbind(`Big 5__C_thorough.2005`, `Big 5__C_lazy.2005`, `Big 5__C_efficient.2005`), na.rm = T)) 
...
```{r b5 composites 2}
 soep_long %>%
  ...
  group_by(Procedural__SID, trait, year) %>% # group by person, construct, & year
