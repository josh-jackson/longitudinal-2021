...
But my preferred approach is to use the modelr package. It is user friendly and it plays nicely with creating pre-specified values ie a range of time values rather than everyone's x-values. The above approaches do well in terms of predicting everyone's x values using the existing dataframe. 

There are two parts to feeding values into an equation. First we have to create a new dataframe for those values to go. If we are *not* going to  use original data, then the fitted values cannot go into the original dataframe. So we need to make it up. To do so we are going to use the data_grid function from modelr. It is very similar to the expand.grid or expand functions if you are familiar with those (I believe data_grid is a wrapper for the expand function). 
