...
-.155 is the association between starting values and how someone changed. Because we are controlling for that, our change estimates will not be uncorrelated with T1. 

## Conditional Models

What if we want to predict initial status (or control for a covariate) as well as see if change is predicted by some variable? 
...
This approach works with categorical as well as continuous predictors. 
...
If you want to get at a standard interpretation of a growth curve, separate from change at a specific time point, you can define the change score with loadings of 1 for all difference score variables as opposed to increasing linearly eg 1,2,3,4,5...
...
Similar to the above models but are additive in their interpretation. That is, the difference score between waves can be thought of adding or subtracting cumulatively from the previous wave. This is a closer hybrid between the growth model and the latent difference score model as the final difference score estimate is interpreted as across the entire time period. 
...
The real benefit of this type of framework is building on this type of change model to incorporate multiple change models simultaneously. This allows one to look at multiple change processes in a single model, opening the door to analyzing more dynamic processes.
...
```{r}
knitr::include_graphics("F5.large.jpg")
```


With coupling parameters you can see how change is associated with change at different levels. For example, think about changes in memory and brain size. As you decrease on memory, lets say you decrease on brain size. However, this may not occur for all levels of memory, or all levels of brain size. Much like an interaction, how you change depends on something. This time it depends it depends on where you are at a particular level of these two variables 
...
# RI-CLPM and a unifying framework
...
## 1. Do you want to differentiate between within person and between person effects? 




## 2. Do you want to account for growth across time? Or do you want to model it wave to wave. 



## 3. How do you want to account for measurement error? 
